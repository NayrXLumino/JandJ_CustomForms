...
public partial class Transactions_TimeModification_lookupSWRepShift : System.Web.UI.Page
{
    ...
    private void populateInclude()
    {
        if (!hfValue.Value.Trim().Equals(string.Empty))
        {
            ...
            string sql = string.Format(@" SELECT DISTINCT Swt_ShiftCode [Shift Code]
                                            ...
                                            LEFT JOIN T_ShiftCodeMaster
                                              ON Swt_ShiftCode = Scm_Shiftcode
                                           WHERE Swt_ShiftCode {0}", sqlINFormat(hfValue.Value));
    ...
    protected string SQLBuilder()
    {
        ...
        sql.Append(@"declare @startIndex int;
                        ...
                        WITH TempTable AS (
                      SELECT Row_Number() OVER (Order by [Shift Code]) [Row], *
                        FROM ( ");
        ...
        sql.Append(" SELECT COUNT(DISTINCT Swt_ShiftCode) [cnt]");
        sql.Append(getFilters());
        sql.Append(") AS Rows");
        return sql.ToString();
    }
    ...
    private string getColumns()
    {
        ...
        columns = @" SELECT DISTINCT Swt_ShiftCode [Shift Code]
                          , Scm_ShiftDesc [Description]
                          , LEFT(Scm_ShiftTimeIn,2) + ':' + RIGHT(Scm_ShiftTimeIn,2)
			                + '-'
			                + LEFT(Scm_ShiftBreakStart,2) + ':' + RIGHT(Scm_ShiftBreakStart,2)
    ...
    private string getFilters()
    {
        ...
        filter = @" FROM T_EmployeeStraightWork
                    LEFT JOIN T_ShiftCodeMaster ON Swt_ShiftCode = Scm_ShiftCode 
                   WHERE 1=1 ";
        ...
        searchFilter = @" AND  ( ( Swt_ShiftCode LIKE '{0}%' )
                              OR ( Swt_ShiftDesc LIKE '{0}%')
                              OR (
			                        + LEFT(Scm_ShiftTimeIn,2) + ':' + RIGHT(Scm_ShiftTimeIn,2)
			                        + '-'
			                        + LEFT(Scm_ShiftBreakStart,2) + ':' + RIGHT(Scm_ShiftBreakStart,2)
