...
public partial class Transactions_Overtime_pgeOvertimeReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else if (!MGBL.getAccessRights(Session["userLogged"].ToString(), "WFOTREP"))
        {
    ...
    protected void btnGenerate_Click(object sender, EventArgs e)
    {
        ...
        UpdatePagerLocation();
        //MenuLog
        //SystemMenuLogBL.InsertGenerateLog("WFOTREP", true, Text.ToString().ToUpper(), Session["userLogged"].ToString(), "");
        SystemMenuLogBL.InsertGenerateLog("WFOTREP", txtEmployee.Text, true, Session["userLogged"].ToString());
        ...
        if (dgvResult.Rows.Count > 0)
        {
            ...
            if (!CommonMethods.isEmpty(ds))
            {
                ...
                try
                {
                    ...
                    ctrl[1] = tempGridView;
                    ExportExcelHelper.ExportControl2(ctrl, "Overtime Report");
                }
                catch (Exception ex)
                {
                    CommonMethods.ErrorsToTextFile(ex, Session["userLogged"].ToString());
    ...
    private string SQLBuilder(string replaceString)
    {
        ...
        sql.Append(@" SELECT SUM(cnt)
                        FROM ( SELECT COUNT(distinct Eot_ControlNo) [cnt]");
                                      sql.Append(getFilters());
        sql.Append(@"           UNION 
                               SELECT COUNT(distinct Eot_ControlNo)");
                                      sql.Append(getFilters().Replace("T_EmployeeOvertime", "T_EmployeeOvertimeHist"));
        sql.Append(@"        ) as Rows");
        ...
        {
            enrouteQuery = @"LEFT JOIN T_EmployeeApprovalRoute on Arm_EmployeeId = Eot_EmployeeId
	                         AND Arm_TransactionID = 'OVERTIME'
                             AND Eot_OvertimeDate BETWEEN T_EmployeeApprovalRoute.Arm_StartDate AND ISNULL(T_EmployeeApprovalRoute.Arm_EndDate, DATEADD(YEAR, 2, GETDATE()))
                             LEFT JOIN T_ApprovalRouteMaster 
                             on T_ApprovalRouteMaster.Arm_RouteId = T_EmployeeApprovalRoute.Arm_RouteID";
            sql = sql.Replace("@Checker1", "ISNULL(Eot_CheckedBy, Arm_Checker1)").Replace("@Checker2", "ISNULL(Eot_Checked2By, Arm_Checker2)").Replace("@Approver", "ISNULL(Eot_ApprovedBy, Arm_Approver)").Replace("@Enroute", enrouteQuery);
        }
        else
        {
            enrouteQuery = @"";
    ...
    private string getColumns()
    {
        ...
        columns = string.Format(@"   SELECT distinct Eot_ControlNo [Control No]
			                              ...
			                              , CASE WHEN Eot_Status = 0 THEN dbo.GetControlEmployeeNameV2(@Approver)
                                                ELSE ''
                                                END [Cancelled By]
                                          , CASE WHEN Eot_Status = 0 THEN Convert(varchar(10), Eot_ApprovedDate,101) 
                                                    + ' ' 
                                                    + RIGHT(Convert(varchar(17), Eot_ApprovedDate,113),5)
                                                ELSE ''
                                                END [Cancelled Date]
                                          , Trm_Remarks [Remarks]
    ...
    private string getFilters()
    {
        ...
        if (tbrEmployee.Visible)
        {
            ...
            {
                filter += string.Format(@" AND (Eot_CostCenter IN (SELECT Cct_CostCenterCode FROM dbo.GetCostCenterLineAccess('{0}', 'OVERTIME') WHERE Clm_LineCode IS NULL OR Clm_LineCode = 'ALL')
									            OR Eot_CostCenter + ISNULL(Eot_CostcenterLine,'') IN (SELECT Cct_CostCenterCode + ISNULL(Clm_LineCode,'') FROM dbo.GetCostCenterLineAccess('{0}', 'OVERTIME'))
                                                OR Eot_EmployeeID = '{0}') ", Session["userLogged"].ToString());
            }
        ...
        if (cbxEnroute.Checked == true)
        {
            filter += @"AND 1 = 1 AND Eot_Status in ('3','5','7')";
        }
...
//    }
    ...
    private void bindGrid()
    {
        ...
        if (!CommonMethods.isEmpty(ds))
        {
            ...
            {
                if (cbxEnroute.Checked == true &&
                        (cblInclude.Items[i].Value == "Checker 1" || cblInclude.Items[i].Value == "Checker 2" || cblInclude.Items[i].Value == "Approver" ||  cblInclude.Items[i].Value == "Cancelled By"))
                    continue;
                if (!cblInclude.Items[i].Selected)
                    ds.Tables[0].Columns.Remove(cblInclude.Items[i].Value);
            }
